...
const schema = z.object({
  ...
  AVNU_PAYMASTER_API_KEY: z.string().min(1).optional(),
  ...
  RELAYER_ENABLED: booleanString.default('false'),
  RELAYER_DAILY_SPEND_LIMIT_GWEI: z.coerce
    .number()
    .int()
    .positive()
...
export function readiness(config: ApiConfig): string[] {
  const required: Array<[keyof ApiConfig, unknown]> = [
    ...
    ['AVNU_PAYMASTER_API_KEY', config.AVNU_PAYMASTER_API_KEY],
  ...
  if (!config.RELAYER_ENABLED) missing.push('RELAYER_ENABLED')
  return missing
}
