...
The three deployed contract addresses are public, immutable configuration and are set directly on
the ECS container from `backend.yaml`. Environment variables override the retained blank bootstrap
fields in the runtime secret, allowing GitHub deployments to update addresses without reading or
rotating any secret value. The public transaction and bytecode provenance is recorded in
`deployments/ethereum-mainnet.json` and `deployments/starknet-mainnet.json`.
